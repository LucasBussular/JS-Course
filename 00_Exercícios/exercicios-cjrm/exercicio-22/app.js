...
const comparador = characters.sort((item1, item2) => item1.id - item2.id)
...
const invertedPeople = people.map( item => item )
...
const joinIngredients = ingredients.reduce( (acc, item, index, array) => {
  let lastLetter = (item) => item[item.length - 1]

  if (index === array.length - 1) {
    acc += `${item} cozido`
  } else if (lastLetter(item) === 'a') {
    acc += `${item} cozida, `
  } else {
    acc += `${item} cozido, `
  }
...
const justDisney = topBrazilmovies.filter( movie => movie.distributedBy === 'Disney')

console.log({justDisney, topBrazilmovies})
...
const moviesHTML = topBrazilmovies
