...
const numberBelow501 = crazyNumbers.filter( number => number < 501)
...
const numbers = [5, 7, 3]
const squaredNumber = numbers.map( item => item * item)
...
const moviesAfter2000 = tarantinoMovies.filter( item => item.release < 2000)
...
const tvShowsNames = tvShows.map( item => {
  return item.name
})

console.log( { tvShows, tvShowsNames } )
