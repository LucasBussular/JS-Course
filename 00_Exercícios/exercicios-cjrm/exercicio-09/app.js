...
const convertLowerCase = string => string.toLowerCase();
...
const getIndex = (caractere, string) => string.indexOf(caractere);
...
const isItThere = (item, array) => array.includes(item)
...
const invokeCallback2 = (value, callback) => {
  return callback(value)
}

invokeCallback2(33, triple)
...
numbers.forEach((item, indez, array) => {
                                            // quando colocado como argumento, o item do método join é o que separa um item do array do outro
...
const fazParagrafo = paragrafo => {
  paragraphs += `<p>${paragrafo}</p>`
}

review.forEach(fazParagrafo)
...
const getLikesMessage = (names = []) => {
  // o switch case recebe como argumento um valor a ser comparado, nesse caso ele recebe o tamanho do array
  switch (names.length) {
    case 0: 
