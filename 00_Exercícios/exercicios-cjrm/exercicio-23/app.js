...
const peopleCopy = people
  .map( ({firstName, lastName, score}) => { return { firstName, lastName, score } })
...
const animalsGraterThan3 = animals.filter( item => item.length === 3)

console.log(animalsGraterThan3)
...
const animalToNumber = animals.map(item => item.length)

console.log(animalToNumber)
...
const friendsNear = friends.filter( friend => friend.nearMe).map( friend => friend.name)
